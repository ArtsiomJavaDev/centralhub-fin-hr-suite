...
# Zlecenie: these PESEL receive Składka chorobowa 2.45% (instead of 0%) with other default rates.
...
def _compact_umowy_typ_alnum(value: object) -> str:
    ...
    if not text or text == "nan":
        return ""
    # Strip remaining combining marks from other Unicode letters (e.g. Cyrillic accents).
...
def _map_umowy_typ_text_to_12(value: object) -> object:
    ...
    if as_digit in ("1", "2"):
        return as_digit
    # Build two compact keys: ascii-only and PL-letters-mapped.
    ascii_key = _normalize_text_value(value)   # ł → stripped entirely
    poly_key = _compact_umowy_typ_alnum(value)  # ł → l, so "dzieło" → "dzielo"
    ...
    for key in (poly_key, ascii_key):
        if not key:
            continue
        # Leading digit — e.g. "1 - Umowa zlecenia"
    ...
    for key in (poly_key, ascii_key):
        if not key:
            continue
        # Substring detection — works now because ł → l in poly_key
...
def _birthdate_from_pesel(pesel: object) -> Optional[date]:
    digits = "".join(ch for ch in _normalize_pesel(pesel) if ch.isdigit())
    if len(digits) < 6:
        return None
    try:
        yy = int(digits[0:2])
